// GENERATED BY THE COMMAND ABOVE; DO NOT EDIT
// This file was generated by swaggo/swag at
// 2020-05-01 09:02:11.579326 +0800 CST m=+4.172346249

package docs

import (
	"bytes"
	"encoding/json"
	"strings"

	"github.com/alecthomas/template"
	"github.com/swaggo/swag"
)

var doc = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{.Description}}",
        "title": "{{.Title}}",
        "contact": {
            "name": "API Support",
            "url": "http://mail.csic711.com/",
            "email": "heyangev@cn.ibm.com"
        },
        "license": {
            "name": "Apache 2.0",
            "url": "http://www.apache.org/licenses/LICENSE-2.0.html"
        },
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/api/v1/create-qr-code": {
            "get": {
                "description": "用于用户绑定多因子",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "多因子"
                ],
                "summary": "生成多因子认证二维码",
                "parameters": [
                    {
                        "type": "string",
                        "description": "数据",
                        "name": "data",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "秘钥",
                        "name": "secret",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "{\"code\": 10000, \"msg\": \"成功\", \"data\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "400": {
                        "description": "{\"code\": 10001, \"msg\": \"参数无效\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "500": {
                        "description": "{\"code\": 60001, \"msg\": \"内部系统接口调用异常\", \"err\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    }
                }
            }
        },
        "/api/v1/login": {
            "post": {
                "description": "用于用户登录",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户登录"
                ],
                "summary": "用户登录",
                "parameters": [
                    {
                        "description": "数据",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.UserLogin"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "{\"code\": 10000, \"msg\": \"成功\", \"data\": \"{\"token\": \"\"}\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "400": {
                        "description": "{\"code\": 10001, \"msg\": \"参数无效\", \"err\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "401": {
                        "description": "{\"code\": 20002, \"msg\": \"账号不存在或密码动态码错误\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    }
                }
            }
        },
        "/api/v1/perm": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "用于创建权限模板",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "权限"
                ],
                "summary": "添加权限模板",
                "parameters": [
                    {
                        "description": "数据",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.SwaggerRole"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "{\"code\": 10000, \"msg\": \"成功\", \"data\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "400": {
                        "description": "{\"code\": 10001, \"msg\": \"参数无效\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "415": {
                        "description": "{\"code\": 50004, \"msg\": \"数据创建错误\", \"err\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    }
                }
            }
        },
        "/api/v1/perm/user/{uid}": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "用于用户关联权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "权限"
                ],
                "summary": "用户关联权限模板",
                "parameters": [
                    {
                        "description": "权限模板id",
                        "name": "role_id",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.AddPermUser"
                        }
                    },
                    {
                        "type": "string",
                        "description": "用户id",
                        "name": "uid",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "{\"code\": 10000, \"msg\": \"成功\", \"data\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "400": {
                        "description": "{\"code\": 10004, \"msg\": \"参数缺失\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "415": {
                        "description": "{\"code\": 50004, \"msg\": \"数据创建错误\", \"err\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    }
                }
            }
        },
        "/api/v1/perm/userGroup/{gid}": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "用于用户组关联权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "权限"
                ],
                "summary": "用户组关联权限模板",
                "parameters": [
                    {
                        "description": "权限模板id",
                        "name": "role_id",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.AddPermUserGroup"
                        }
                    },
                    {
                        "type": "string",
                        "description": "用户组id",
                        "name": "gid",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "{\"code\": 10000, \"msg\": \"成功\", \"data\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "400": {
                        "description": "{\"code\": 10004, \"msg\": \"参数缺失\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "415": {
                        "description": "{\"code\": 50004, \"msg\": \"数据创建错误\", \"err\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    }
                }
            }
        },
        "/api/v1/register": {
            "post": {
                "description": "用于用户的注册",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户注册"
                ],
                "summary": "注册用户",
                "parameters": [
                    {
                        "description": "用户注册数据",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.SwaggerUser"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "{\"code\": 10000, \"msg\": \"成功\", \"data\": models.GoogleAuth}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "400": {
                        "description": "{\"code\": 10001, \"msg\": \"参数无效\", \"err\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "415": {
                        "description": "{\"code\": 50004, \"msg\": \"数据创建错误\", \"err\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    }
                }
            }
        },
        "/api/v1/userGroup": {
            "post": {
                "security": [
                    {
                        "ApiKeyAuth": []
                    }
                ],
                "description": "用于用户组的创建",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户组"
                ],
                "summary": "添加用户组",
                "parameters": [
                    {
                        "description": "用户组数据",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.SwaggerUserGroup"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "{\"code\": 10000, \"msg\": \"成功\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "400": {
                        "description": "{\"code\": 10001, \"msg\": \"参数无效\", \"err\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "415": {
                        "description": "{\"code\": 50004, \"msg\": \"数据创建错误\", \"err\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    }
                }
            }
        },
        "/api/v1/view-qr-code": {
            "get": {
                "description": "用于用户绑定多因子",
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "多因子"
                ],
                "summary": "显示多因子认证二维码",
                "parameters": [
                    {
                        "type": "string",
                        "description": "图片名",
                        "name": "imagename",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {},
                    "400": {
                        "description": "{\"code\": 10001, \"msg\": \"参数无效\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "404": {
                        "description": "{\"code\": 50001, \"msg\": \"数据未找到\", \"err\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    },
                    "500": {
                        "description": "{\"code\": 60001, \"msg\": \"内部系统接口调用异常\", \"err\": \"\"}",
                        "schema": {
                            "$ref": "#/definitions/gin.H"
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "gin.H": {
            "type": "object",
            "additionalProperties": true
        },
        "models.AddPermUser": {
            "type": "object",
            "properties": {
                "role_id": {
                    "type": "string"
                }
            }
        },
        "models.AddPermUserGroup": {
            "type": "object",
            "properties": {
                "role_id": {
                    "type": "string"
                }
            }
        },
        "models.SwaggerObjActId": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "string"
                }
            }
        },
        "models.SwaggerRole": {
            "type": "object",
            "required": [
                "roleObjActs",
                "role_name"
            ],
            "properties": {
                "remark": {
                    "type": "string"
                },
                "roleObjActs": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.SwaggerObjActId"
                    }
                },
                "role_name": {
                    "type": "string"
                }
            }
        },
        "models.SwaggerUser": {
            "type": "object",
            "required": [
                "name",
                "pass_word",
                "user_name"
            ],
            "properties": {
                "email": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "pass_word": {
                    "type": "string"
                },
                "phone": {
                    "type": "string"
                },
                "user_name": {
                    "type": "string"
                }
            }
        },
        "models.SwaggerUserGroup": {
            "type": "object",
            "required": [
                "group_name",
                "users"
            ],
            "properties": {
                "group_name": {
                    "type": "string"
                },
                "remark": {
                    "type": "string"
                },
                "users": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.SwaggerUserId"
                    }
                }
            }
        },
        "models.SwaggerUserId": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "string"
                }
            }
        },
        "models.UserLogin": {
            "type": "object",
            "required": [
                "otp_code",
                "pass_word",
                "user_name"
            ],
            "properties": {
                "otp_code": {
                    "type": "string"
                },
                "pass_word": {
                    "type": "string"
                },
                "user_name": {
                    "type": "string"
                }
            }
        }
    },
    "securityDefinitions": {
        "ApiKeyAuth": {
            "type": "apiKey",
            "name": "Authorization",
            "in": "header"
        }
    }
}`

type swaggerInfo struct {
	Version     string
	Host        string
	BasePath    string
	Schemes     []string
	Title       string
	Description string
}

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = swaggerInfo{
	Version:     "v1.0",
	Host:        "127.0.0.1:8080",
	BasePath:    "/",
	Schemes:     []string{},
	Title:       "WisdomPortal backend API",
	Description: "This is a wisdomPortal backend interface system server.",
}

type s struct{}

func (s *s) ReadDoc() string {
	sInfo := SwaggerInfo
	sInfo.Description = strings.Replace(sInfo.Description, "\n", "\\n", -1)

	t, err := template.New("swagger_info").Funcs(template.FuncMap{
		"marshal": func(v interface{}) string {
			a, _ := json.Marshal(v)
			return string(a)
		},
	}).Parse(doc)
	if err != nil {
		return doc
	}

	var tpl bytes.Buffer
	if err := t.Execute(&tpl, sInfo); err != nil {
		return doc
	}

	return tpl.String()
}

func init() {
	swag.Register(swag.Name, &s{})
}
